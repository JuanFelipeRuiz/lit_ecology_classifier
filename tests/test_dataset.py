...
class TestCreateDataFrame:
    ...
    def test_extract_timestamp_from_filename(self):
        ...
        assert self.create_dataset._extract_timestamp_from_filename(file_name) ==  expected_timestamp.replace(tzinfo=timezone.utc)
    ...
    def test_process_image_correct_output(self):
        ...
        with patch.object(self.create_dataset, '_extract_timestamp_from_filename', return_value='fake_timestamp'), \
    ...
    def test_hotencoding_group_by(self):
        ...
        pd.testing.assert_frame_equal(self.create_dataset._add_one_hot_encoded_versions_and_group_by(df), expected_output), 'The output is not correct'
    ...
    def test_split_overview_from_txt(self, mock_exists):
        ...
        with patch('numpy.loadtxt') as mock_loadtxt:
            ...
            assert self.create_dataset._load_split_overview_from_txt('1') == expected_output
    ...
    def test_load_split_overview_from_pickle(self, mock_exists):
        with patch('pandas.read_pickle') as mock_read_pickle:
            ...
            assert self.create_dataset._load_split_overview_from_pickle('2') == expected_output
    ...
    def test_add_split_group_column(self):
        ...
        pd.testing.assert_frame_equal(self.create_dataset._add_split_column(df=input_df, image_paths=train_lst , split_name='train_v1'), expected_output)
    ...
    def test_prepare_split_paths(self, mock_function_pickle, mock_function_txt, version, should_call_txt, should_call_pickle, should_warn):
        ...
        with patch.object(self.create_dataset, 'zoolake_version_paths', new={version: '_'}):
            ...
            if should_warn:
                ...
                with pytest.warns(UserWarning, match="New version, assuming a pickle file for split in the folder"):
    ...
    def test_function_called_with_correct_columnname(self, mock_function):
        ...
        self.create_dataset._apply_splits_to_dataframe(images_paths_split=input_image_paths_split, version=version, df=df)
