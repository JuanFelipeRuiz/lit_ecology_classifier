...
class BaseImageMover:
    """
    Base class for moving images from a source directory to a destination directory.
    ...
    def __init__(self, src_base_path: str = None, tgt_base_path: str = None):
        """
        Initialize the BaseImageMover with source and destination base paths.
    ...
    def _prepare_target_folders(self, move_df: pd.DataFrame):
        """
        Prepare the target folders for the images to be moved.

        Filters all unique target folders and applies the function to create a folder to each one.

        Args:
            move_df (pd.DatFrame): Dataframe containing the target path
        """
        ...
        list(map(lambda tgt_folder: self.create_tgt_folder(str(tgt_folder)), unique_tgt_folders))
    ...
    def _check_input(self, paths: Union[pd.DataFrame, List[Tuple[str, str]]]):
        ...
        if isinstance(paths, pd.DataFrame):
            ...
            move_df = paths[["src", "tgt"]].copy()
        elif isinstance(paths, list):
            move_df = pd.DataFrame(paths, columns=["src", "tgt"])
        else:
            raise TypeError("paths must be a DataFrame or a list of tuples.")

        logger.info("Number of images to move: %s", len(move_df))

        return move_df

    def _iterate_over_paths(self, move_df: pd.DataFrame):
        """Simple iteration over the src and targets paths to move the images

        Args:
            move_df (pd.DataFrame): Df with target and souce paths
        ...
        for _, row in move_df.iterrows():
            ...
            try:
                self.copy_image(src_path, tgt_path)
                success_count += 1
            except ValueError:
                failure_count += 1

        logger.info("%s of %s images moved successfully.",success_count, len(move_df))
        if failure_count > 0:
            logger.error("%s images failed to move.", failure_count)
            raise ValueError

    def _parallel_image_mover(self, move_df: pd.DataFrame, max_workers: int = None):
        """Moves the images with parallesization to make the copy faster.

        Args:
            move_df (pd.DataFrame): dataframe containg the source and target path as columns
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(self.copy_image, row["src"], row["tgt"])
    ...
    def move_images(
        ...
        """
        Moves images based on a DataFrame or list of tuples containing source and target paths.
        ...
        Args:
            ...
            parallel (bool): Whether to move images in parallel. Defaults to False.
            max_workers (int): Maximum number of workers for parallel processing.

        """
        move_df = self._check_input(paths)

        self._prepare_target_folders(move_df)

        if self.src_base_path is not None:
            move_df = self._append_src_base_path(move_df)

        if parallel:
            self._parallel_image_mover(move_df, max_workers)
        else:
            self._iterate_over_paths(move_df)
