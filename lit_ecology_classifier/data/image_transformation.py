...
class ResizeWithProportions:
    ...
    def _check_dimensions(self):
        ...
        if self.width and self.height:
            largest_dim = max(self.width, self.height)
            smallest_dim = min(self.width, self.height)
            if (self.target_size * smallest_dim) / largest_dim < 5:
                warnings.warn("The image dimensions exceed the maximum allowed ratio of 5:1")
    ...
    def _shrink_image(self, image: Image.Image) -> Image.Image:
        ...
        Returns: 
            The resized image.
        """
        ratio = float(self.target_size) / max(self.width, self.height)
    ...
    def _add_padding(self, image: Image.Image) -> Image.Image:
        ...
        paste_position = ((self.target_size - image.size[0]) // 2, (self.target_size - image.size[1]) // 2)
    ...
    def __call__(self, input_object: Image.Image) -> Image.Image:
        ...
        """
        image = self._validate_input(input_object)
        self.width, self.height = image.size

        self._check_dimensions()

        if max(self.width, self.height) > self.target_size:
            image = self._shrink_image(image)

        return self._add_padding(image)
...
def define_resize_transformation(
    ...
    """
    return [ResizeWithProportions(target_size)] if resize_with_proportions else [Resize(target_size)]
...
def define_transformation_pipeline(
    ...
    Args:
        train: Flag indicating whether the transformation pipeline is for training or validation/testing to define the transformation.
    ...
    resize_transformation = define_resize_transformation(target_size=target_size, resize_with_proportions= resize_with_proportions)
...
if __name__ == "__main__":
    ...
    ## preparations
    pipeline_with_porpoptions = define_transformation_pipeline(train=False, target_size = target_size, resize_with_proportions=True)
    pipeline_without_porpoptions = define_transformation_pipeline(train=False, target_size = target_size, resize_with_proportions=False)
    ...
    def create_pipelines(augmentation_level, resize_with_proportions, image):
        pipeline = define_transformation_pipeline(target_size= target_size, resize_with_proportions=resize_with_proportions, train=True, augmentation_level=augmentation_level)
