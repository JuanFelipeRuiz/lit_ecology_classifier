...
class CreateOverviewDf:
    ...
    def find_images_in_folder(self, folder_path):
        ...
        return [
            ...
            # walk through the folder and subfoledrs (generates lists)
    ...
    def check_duplicates(self, df):
        ...
        if self.hash_algorithm == "sha256":
            ...
            self.duplicates_hashes = df[
                df.duplicated(
                    subset=["data_set_version", self.hash_algorithm], keep=False
                )
            ][self.hash_algorithm].tolist()

            if len(self.duplicates_hashes) > 0:
                return self.duplicates_hashes
                raise Warning(
                    f"Duplicates found in the dataset: {len(self.duplicates_hashes)}"
                )
    ...
    def load_test_train_from_txt(self, datapath):
        filenames_train = np.loadtxt(
            datapath + "/zoolake_train_test_val_separated/train_filenames.txt",
            dtype=str,
        )
        filenames_val = np.loadtxt(
            datapath + "/zoolake_train_test_val_separated/val_filenames.txt", dtype=str
        )
        filenames_test = np.loadtxt(
            datapath + "/zoolake_train_test_val_separated/test_filenames.txt", dtype=str
        )
    ...
    def update_each_test_train_split(self, train_test_val_array):
        ...
        for (versions,) in enumerate(train_test_val_array):
            for i, split in enumerate(train_test_val_array):
                ...
                self.update_df_with_split_image_names(
                    df, train_test_val_array[i], splits[i]
                )
