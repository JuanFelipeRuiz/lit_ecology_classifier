...
from .overview.image_processing import ProcessImage
from .overview.raw_split_preparer import RawSplitPathPreparer
from .overview.raw_split_applier import RawSplitApplier


class OverviewCreator:
    """Create an overview of the images in the difffrent ZooLake dataset versions 

    The OverviewCreator is used to create an overview of the images based on the diffrent ZooLake dataset versions that
    are provided. 
    """
    def __init__(self, zoolake_version_paths: dict = None, hash_algorithm: str = None):
        ...
        self.zoolake_version_paths = self._init_zoolake_version_paths(zoolake_version_paths)
    ...
    def _prepare_image_paths(self) -> list[dict]:
    ...
    def _process_images_by_version(self) -> list[dict]:
        ...
        # Prepare ProcessImage class to process the images
        image_proccessor = ProcessImage(hash_algorithm=self.hash_algorithm)

        # Process all images using map
        processed_images = map(
            lambda version_image: image_proccessor.process_image(*version_image),
            version_image_pairs
        )
    ...
    def _ood_fixes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df["data_set_version"].str.contains("OOD").any():
            warnings.warn("Found 'OOD' in 'data_set_version' column. Assuming Version 2")
            ...
            df["data_set_version"] = df["data_set_version"].str.replace("OOD", "2", regex=False)
            
        return df  

    
    def _add_one_hot_encoded_versions_and_group_by(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _add_split_columns(self, df: pd.DataFrame, split_paths: dict) -> pd.DataFrame:
        split_applier = RawSplitApplier(split_paths)
    ...
    def main(self, load_new=False):
        ...
        split_paths = RawSplitPathPreparer(self.zoolake_version_paths).prepare_split_paths()
