...
def setup_model(
    ...
    **kwargs,
    ...
    if architecture in model_mapping:
        model = timm.create_model(model_mapping[architecture], pretrained= pretrained, num_classes=num_classes)
    else:
        raise NotImplementedError(f"Architecture {architecture} could not be prepared from huggingface.")
    ...
    print("Total number of trainable parameters: %d", sum(p.numel() for p in model.parameters() if p.requires_grad))
